...
gulp.task("sass", function () {
  return gulp.src("./src/styles/**/*.scss").pipe(sass.sync({ outputStyle: "expanded" }).on("error", sass.logError)).pipe(autoprefixer()).pipe(gulp.dest("./dist/css"));
});

gulp.task("demo-sass", function () {
  return gulp.src("./demo/main.scss").pipe(sass.sync().on("error", sass.logError)).pipe(autoprefixer()).pipe(gulp.dest("./demo"));
});
